...
import { BarChart3, Bell, BookOpen, ChevronLeft, ClipboardList, Home, Languages, LogOut, Mail, Menu, Search, Settings, ShoppingCart, Users, X } from 'lucide-react';
...
export function AppLayout() {
  ...
  const nav = <>
    <div className="flex items-center justify-between border-b border-border px-5 py-5">
      ...
      <button className="hidden rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground lg:block" onClick={() => setCollapsed(!collapsed)} aria-label="Toggle sidebar"><ChevronLeft className={`h-4 w-4 transition-transform ${collapsed ? 'rotate-180' : ''}`} /></button>
  ...
  return <div className="min-h-screen bg-background text-foreground md:flex"><aside className={`hidden border-r border-border bg-card transition-all lg:flex lg:min-h-screen lg:flex-col ${collapsed ? 'lg:w-[84px]' : 'lg:w-64'}`}>{nav}</aside>{mobileOpen && <div className="fixed inset-0 z-50 lg:hidden"><button className="absolute inset-0 bg-black/70" onClick={closeMobile} aria-label="Close navigation" /><aside className="relative flex h-full w-72 flex-col border-r border-border bg-card">{nav}</aside></div>}<main className="min-w-0 flex-1"><header className="sticky top-0 z-30 flex h-[58px] items-center gap-3 border-b border-border bg-background/95 px-4 backdrop-blur-md md:px-6"><button className="rounded-lg p-2 text-muted-foreground hover:bg-muted lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open navigation" aria-expanded={mobileOpen}><Menu className="h-5 w-5" /></button><button className="hidden rounded-lg border border-border p-2 text-muted-foreground hover:bg-muted lg:block" onClick={() => setCollapsed(!collapsed)} aria-label="Toggle sidebar"><Menu className="h-4 w-4" /></button><div className="relative hidden w-full max-w-[300px] md:block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input aria-label="Search" placeholder="Search..." className="h-9 w-full rounded-md border border-border bg-card pl-9 pr-3 text-sm outline-none placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary" /></div><div className="ml-auto flex items-center gap-1"><span className="hidden text-sm text-muted-foreground lg:block">{isStudent ? 'Student workspace' : 'Teacher workspace'}</span><div className="mx-1 hidden h-5 w-px bg-border sm:block" /><IconButton label="Change language"><Languages className="h-4 w-4" /></IconButton><IconButton label="Cart"><ShoppingCart className="h-4 w-4" /><span className="absolute -right-0.5 -top-0.5 flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-primary px-1 text-[9px] text-primary-foreground">2</span></IconButton><IconButton label="Notifications"><Bell className="h-4 w-4" /><span className="absolute right-0 top-0 h-1.5 w-1.5 rounded-full bg-rose-500" /></IconButton><IconButton label="Messages"><Mail className="h-4 w-4" /></IconButton><ThemeToggle /><div className="ml-1 hidden h-7 w-px bg-border sm:block" /><button className="ml-1 flex items-center gap-2 rounded-md p-1.5 hover:bg-muted" aria-label="Open profile menu"><Avatar id={user?.username || 'guest'} name={displayName} size="sm" seedAvatar={false} /><span className="hidden max-w-24 truncate text-sm font-medium xl:block">{displayName}</span></button></div></header><div className="mx-auto max-w-[1440px] p-4 md:p-8"><Outlet /></div></main></div>;
}
